param(
  [string]$RoutePath = 'dictionaries\countries',  # or 'countries' if you want /countries
  [string]$ProjectRoot
)
...
$targetDir = Join-Path $ProjectRoot ("app\" + $RoutePath)
...
export default function CountriesTable({
  ...
  // Local state for filter inputs (so we can debounce)
  ...
  const buildUrl = useCallback(
    (patch: Record<string, string | number | undefined>) => {
      ...
      // Always reset to first page when filters/pageSize change
  ...
  const go = (p: number) => router.push(buildUrl({ page: Math.min(Math.max(1, p), lastPage) }));
          ...
          <tbody className="text-sm">
            {rows.map((r) => (
              <tr key={r.id} className="border-b last:border-0 hover:bg-gray-50">
                ...
                <td className="px-3 py-2">{`${r.name_ka} - ${r.iso3}`}</td>
              </tr>
            ))}
            {rows.length === 0 && (
              <tr>
...
export default async function Page({
  ...
  // BigInt → number for Client Component
...
Set-Content -Path (Join-Path $targetDir 'CountriesTable.tsx') -Value $countriesTable -Encoding UTF8
Set-Content -Path (Join-Path $targetDir 'page.tsx') -Value $pageTsx -Encoding UTF8

Write-Host "✓ Wrote CountriesTable.tsx and page.tsx to $targetDir"
Write-Host "Open http://localhost:3000/$($RoutePath -replace '\\','/') to view."
