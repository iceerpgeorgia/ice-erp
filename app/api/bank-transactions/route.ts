  ...
  try {
    ...
    if (!idsParam && !rawRecordUuid) {
      ...
      conversionRows = conversionFiltered.map((row) => ({
        ...
        dockey: row.dockey ?? null,
        entriesid: row.entriesid ?? null,
        transaction_date: toComparableDate(row.transaction_date ?? null) ?? row.transaction_date ?? null,
        correction_date: toComparableDate(row.correction_date ?? null) ?? row.correction_date ?? null,
    ...
    const resultWithConversions = [...result, ...conversionRows];
    ...
    const fetchedIds = new Set(
      [...filteredTransactions.map(t => t.synthetic_id ?? t.id), ...conversionRows.map(r => r.id)]
