...
- The Handovers jobs table shows calculated columns for Paid Nominal, Paid GEL, Debit Nominal, Debit GEL, and Total GEL, and its footer totals the numeric business columns that are actually displayed as totals (for example Floors and the money columns, but not Weight). These values are computed from income-only job distributions, and GEL conversion uses the job's `liftCertDate` with the project currency's NBG rate lookup by date.
