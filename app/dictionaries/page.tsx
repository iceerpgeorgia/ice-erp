import Link from "next/link";

export const revalidate = 0;

export default function DictionariesIndex() {
  return (
    <div className="w-full">
      <div className="mx-auto max-w-[800px] px-6 py-10">
        <h1 className="text-2xl font-semibold mb-6">Dictionaries</h1>
        <ul className="list-disc ml-6 space-y-2">
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/countries">
              Countries
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/entity-types">
              Entity Types
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/counteragents">
              Counteragents
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/currencies">
              Currencies
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/banks">
              Banks
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/bank-accounts">
              Bank Accounts
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/bank-transactions">
              Bank Transactions
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/bank-transactions-test">
              Bank Transactions Test
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/bank-transaction-batches">
              Bank Transaction Batches
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/parsing-scheme-rules">
              Parsing Scheme Rules
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/nbg-rates">
              NBG Exchange Rates
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/admin/financial-codes">
              Financial Codes
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/admin/projects">
              Projects
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/admin/attachments">
              Attachments
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/admin/document-types">
              Document Types
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/jobs">
              Jobs
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/payments">
              Payments
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/payments-ledger">
              Payments Ledger
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/payments-report">
              Payments Report
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/services-report">
              Services Report
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/projects-report">
              Projects Report
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/conversions">
              Conversions
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/waybills">
              Waybills In
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/payment-redistribution">
              Payment Redistribution
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/salary-accruals">
              Salary Accruals
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/dimensions">
              Dimensions
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/inventory-groups">
              Inventory Groups
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/inventories">
              Inventories
            </Link>
          </li>
          <li>
            <Link className="text-blue-600 hover:underline" href="/dictionaries/waybill-items">
              Waybill Items
            </Link>
          </li>
        </ul>
      </div>
    </div>
  );
}
