...
def fetch_database_dictionaries():
    ...
    try:
        ...
        print(f"   [OK] Financial codes: {len(financial_codes)} records ({active_fc} active)")
...
def extract_inventory_code(inventory_value):
    """Extract code from composite inventory string."""
...
def extract_financial_code_prefix(fc_value):
    """Extract code from financial code string."""
    if not fc_value:
        return None
...
def validate_data(waybill_data, db_dicts):
    ...
    print(f"\n   Detected columns:")
    print(f"     Project column (idx {project_col_idx}): {headers[project_col_idx][:50] if project_col_idx and project_col_idx < len(headers) else 'Not found'}")
    print(f"     Financial Code column (idx {fc_col_idx}): {headers[fc_col_idx][:50] if fc_col_idx and fc_col_idx < len(headers) else 'Not found'}")
    print(f"     Inventory column (idx {inv_col_idx}): {headers[inv_col_idx][:50] if inv_col_idx and inv_col_idx < len(headers) else 'Not found'}")
    ...
    for row_idx, row in enumerate(data, start=2):
        ...
        if project_col_idx is not None and project_col_idx in row:
            ...
            if project_val and project_val is not True:
...
def generate_report(waybill_data, db_dicts, issues):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        json.dump({
            ...
            'summary': summary,
            'issues': {
                'missing_projects': issues['missing_projects'][:100],
...
def main():
    ...
    if all_clean:
        ...
        print("\n[ALERT] Issues found. Review the report above and detailed JSON file.")
